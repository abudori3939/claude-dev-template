...
> **いつ通すか（グリーンで止まらないフローとの関係）**
> CLAUDE.md のフローではグリーン後に止まらず PR 作成・レビュー起動まで進むが、**実機ゲートはその「マージ前」側に残る人間ゲート**である。
> したがって **PR を作成した時点で、下記 1. の手順と合否基準を PR 本文またはコメントに書いて人間に依頼する**。コードレビューと**並行**して人間に実施してもらい、**ログと判定が PR に記録されるまでマージしない**。
> 実機ゲート未実施の PR をマージしてはならない（native テストが緑でもマージ条件を満たさない）。

1. **エージェント**: `pio test -e native` 緑 ／ `pio run -e pico` `-e pico2` 緑 を確認し、
   **PR 作成と同時に**人間向けに次を提示する。
...
- ビルド警告ゼロ（自分のコードは `-Werror`。core / 外部ライブラリの警告で落とさないよう `build_src_flags` に置く）。PoC モードでも**実機ゲート・対象 MCU 全ビルド・自分のコードの `-Werror` は緩めない**（`agent_docs/project/plan.md`「PoC でも緩めないもの」）。「PoC は warning 非門」は外部ツールの警告に対する緩和であり、`-Werror` によるビルド失敗はそのまま門として残す。
...
**PR を出す前**に自己点検する項目（最後の 2 つを除く全部）と、**マージ前**に満たす項目（★）に分かれる。実機ゲートは人間がログを PR に貼る工程なので、PR 作成前には完了しない。
...
- [ ] PR 作成時に**実機ゲートの手順と合否基準を提示**し、人間に依頼した
- [ ] ★**（マージ前）実機ゲートのログが PR にあり、判定結果を記録した**
- [ ] `progress.md` を更新した
