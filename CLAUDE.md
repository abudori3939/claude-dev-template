...
5. **リファクタリング**: テストが緑のまま、重複除去・命名改善・責務整理を行う。掃除も同じ PR に含め、「動くが汚い」コードを次サイクルへ持ち越さない。
   - **純リファクタだけの PR**（外部から観測できる振る舞いを変えない）は、**新しいレッドを作らない**。既存テストが緑のままであることが合格条件。
...
> **ファームウェア（RP2040 / RP2350）スタックの場合**: フロー自体は通常どおり（レッド＝**ホスト側 native ユニットテスト**のアサーション失敗）。ただし**エージェントは実機を持たない**ため、サイクルの最後に**実機ゲート**（人間が書き込み・動作確認し、ログを PR に貼る）を必ず通す。ロジックをハード非依存に分離する規約を含め `agent_docs/stacks/rp2xxx_platformio.md` に従う。
...
- **グリーンで止まらない。** 実装（グリーン＋リファクタ）が終わったら、サマリを報告したうえで**ユーザーの返答を待たずに PR を作成し、別エージェントによるコードレビューを開始する**。レビューは時間がかかるため、先に走らせてユーザーの確認と並行させる。
  - **減らすのは「グリーン後・PR 作成前」の停止だけ。** それより前のゲート（**着手ゲート** / **レッド確認** / **テスト承認**）は従来どおり全部残る。**実装完了以降**の承認ゲートが「マージ前」1 か所になる、という意味であって、TDD のゲートを省いてよいという意味ではない。
  - スタック固有の**人間ゲート**（KiCad のフットプリント照合、RP2040/RP2350 の**実機ゲート**など）も残る。これらは PR 作成時に手順を提示して**レビューと並行して依頼し、マージ前に必ず通す**。
  - **次のいずれかに当てはまる場合だけは、PR を作る前に一度ユーザーに確認する**（前提がずれたままレビューを回すとレビュアーの時間が無駄になるため）:
    ...
    - 上記に当てはまらないなら止まらない。「なんとなく不安だから」で止めない。
- CI が緑で、ユーザーが問題ないと確認したらマージする（**PR で合格したコードのみマージ**）。
- **プロジェクト完了まで、この小さなフロー（着手確認 → 計画 → レッド確認 → テスト承認 → グリーン → 報告して続行 → PR 作成 → レビュー → 指摘対応 → マージ）を繰り返す。**
...
### エージェント分業とモデル選択
親エージェントが高性能・高コストなモデル（Opus / Fable など）で動くとき、**すべてを親でやらない**。詳細は **`agent_docs/common/agent_delegation.md`**。

- **親は「監督・判断・対話」に専念する。** 仕様・設計の判断、ユーザーとの確認、TDD の方針決定は親が持つ。
- **判断の要らない実作業は、軽量モデル（Haiku / Sonnet）のサブエージェントに降ろす。** 大量リネーム・定型置換、差分を読んでの PR 本文起草、確定した指摘の機械的な適用、長いログ・PDF の読解と要約など。サブエージェントには**要約（テキスト）だけを返させ**、生データを親のコンテキストに持ち込ませない。
- **決定的な作業はモデルにやらせない。** ビルド・テスト・lint・出力生成はスクリプト／コマンドで実行する。`gh pr create` のような**単発の決定的コマンドは親がそのまま撃つ**（サブエージェントを起こすほうが高くつく）。
- **コードレビューだけは軽量化しない。** レビューは判断そのものなので、実装者とは別エージェント・**同等以上のモデル**で行う。軽くするのはレビューの前後であって、レビュー自身ではない。
...
- **PoC モードでは CI を「build ＋ 境界テスト ＋ format ＋『PoC でも緩めないもの』のうち自動化できる検査」に絞る**（ソフトウェアスタックは目標3分以内、ビルドが重いスタックは「毎 PR に必要な最小構成」に）。遅い CI と増え続けるテストは PoC の速度を殺し、検証に入れないまま終わる原因になる（`plan.md`「開発モード」）。
...
例: ROS 2 / C++ は `agent_docs/stacks/ros2_cpp.md`（colcon / gtest / ament_lint）、KiCad 基板設計は `agent_docs/stacks/kicad.md`（kicad-cli / ネットリスト pytest / ERC・DRC）、RP2040 / RP2350 ファームウェアは `agent_docs/stacks/rp2xxx_platformio.md`（pio run / pio test -e native / pio check）。
